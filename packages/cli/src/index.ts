...
function outputHtml(result: ScanResult): string {
  const findingRows = result.findings.map(f => `
    <tr>
      <td><span class="sev sev-${escapeHtml(f.severity)}">${escapeHtml(f.severity.toUpperCase())}</span></td>
      <td>${escapeHtml(f.category)}</td>
      <td>${escapeHtml(f.title)}</td>
      <td>${escapeHtml(f.filePath)}:${escapeHtml(f.lineNumber || '-')}</td>
...
<head>
  ...
  <title>AI Skill Shield CLI Report</title>
  <style>
...
<body>
  <h1>AI Skill Shield CLI Report</h1>
  <div class="score">${result.score}/100</div>
  <div class="meta">Highest Finding: ${escapeHtml(result.riskLevel.toUpperCase())} &middot; ${result.findings.length} findings</div>
  <h2>Findings</h2>
  ...
  <div class="footer">Generated by AI Skill Shield CLI</div>
</body>
</html>`;
}
...
function outputSarif(result: ScanResult): string {
  const runs = [{
    tool: { driver: { name: 'AI Skill Shield', informationUri: 'https://github.com/skill-shield', rules: [] } },
...
function outputMarkdown(result: ScanResult): string {
  let md = `# AI Skill Shield Scan Report\n\n`;
  md += `**Score:** ${result.score}/100\n`;
  md += `**Highest Finding Severity:** ${result.riskLevel.toUpperCase()}\n`;
...
program
  ...
  .action(async (skillPath: string, options: { format: string; failOn: string; output?: string }) => {
